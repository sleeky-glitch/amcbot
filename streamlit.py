...
def generate_response(input_text):
    ...
    if isinstance(response, str):
        ...
        response = response.replace("\\n", "\n").replace("\t", " ")  # Handle newlines and tabs
        response = response.replace("Guj", "Gujarat")  # Expand abbreviations if needed

        # Split response by numbered sections or sentences for better readability
        response_parts = response.split("\n")
        formatted_response = []
        current_part = ""
        
        for part in response_parts:
            # Detect start of a new numbered section (assumes sections start with single digits or "404.", "405.", etc.)
